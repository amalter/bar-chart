 ...
 * @since 1.0.0
 */
function bar_chart_cgb_block_assets() { // phpcs:ignore
	// Register block styles for both frontend + backend.
	wp_register_style(
		'bar_chart-cgb-style-css', // Handle.
	...
	wp_register_style(
		'bar_chart-cgb-block-editor-css', // Handle.
		plugins_url( 'dist/blocks.editor.build.css', dirname( __FILE__ ) ), // Block editor CSS.
		array( 'wp-edit-blocks' ), // Dependency to include the CSS after it.
		null // filemtime( plugin_dir_path( __DIR__ ) . 'dist/blocks.editor.build.css' ) // Version: File modification time.
	);
	//only register if on frontend
	if( !is_admin() ) {
	 ...
	 */
	register_block_type(
		'cgb/block-bar-chart', array(
			// Enqueue blocks.style.build.css on both frontend & backend.
			'style'         => 'bar_chart-cgb-style-css',
...
// Hook: Block assets.
add_action( 'init', 'bar_chart_cgb_block_assets' );
